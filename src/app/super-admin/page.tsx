...
export default function SuperAdminPage() {
  ...
  const handlePasswordUpdate = async (id: string) => {
    ...
    await fetch("/api/super-admin/tenants", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, adminPassword: newPassword }),
    });
    setEditingId(null);
    setNewPassword("");
    load();
                  ...
                  {editingId === t.id ? (
                    <div className="flex gap-2 items-center">
                      <input
                        ...
                        className="bg-gray-700 border border-gray-600 rounded-lg px-2 py-1 text-xs text-white w-36"
                      />
                      <button onClick={() => handlePasswordUpdate(t.id)}
                        className="text-xs bg-amber-600 text-white px-2 py-1 rounded-lg">更新</button>
                      <button onClick={() => { setEditingId(null); setNewPassword(""); }}
                        className="text-xs bg-gray-600 text-white px-2 py-1 rounded-lg">キャンセル</button>
                    </div>
                  ) : (
